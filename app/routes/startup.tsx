...
export default function Startup () {
  ...
  function handleStartupVideoTimeUpdate (event: SyntheticEvent<HTMLVideoElement>) {
    ...
    // NOTA: Esto es para acelerar tiempos por ahora
  ...
  return (
    <>
      <button
        className={`${isConsoleOn ? 'consoleOn' : ''}
          [.consoleOn]:opacity-0 [.consoleOn]:pointer-events-none [.consoleOn]:select-none
          absolute h-full w-full cursor-pointer outline-0 z-2 transition-all
        `}
      ...
      <section className='h-full w-full flex items-center justify-center bg-[#1A1A1A]'>
        <video
          ref={startupVideoRef}
          src='/switchui/movies/startup.mp4'
